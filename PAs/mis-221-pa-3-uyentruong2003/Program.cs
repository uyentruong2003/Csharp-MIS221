...
static void GuessTenMain (ref int credit){
    ...
    System.Console.Write("Click any key to draw 1 random card and a row of ten...");
    Console.ReadKey();
    Console.Clear();
    System.Console.WriteLine();
    ...
    System.Console.WriteLine($"Here is the random card: {randomCard}");
...
static int PlayGuessTen(string randomCard, string[] rowOfTen, string[] valueOrder, string[] suitOrder, ref string[] shownValues, ref string[] shownSuits){
    ...
    // get guess of the first card:
    int index = 0; //prime read
    int num = index +1;
    int wins = 0;
    
    string nextCard = rowOfTen[index];
    string guess = GetGuess(num, currentCard);

    //check guess:
    bool keepGoing = IsCorrect(1, guess, currentCard, nextCard, valueOrder, suitOrder, ref shownValues, ref shownSuits); //prime read
    if(keepGoing==true) wins++;

    // Proceed to next round when the player guesses correctly
    while ( keepGoing == true && index < rowOfTen.Length-1){
        FlipCardInRow(index, currentCard, ref shownValues, ref shownSuits);
        // move up to the next card:
        index ++; //update read
        num = index +1;
        currentCard = rowOfTen[index++];
        System.Console.WriteLine($"\n ROUND {num++}:");
        System.Console.WriteLine($"\nCard #{num++}: {currentCard}");
        

        // get guess of the next card:
        nextCard = rowOfTen[index + 1];
        guess = GetGuess(num+1, currentCard);

        // check guess:
        keepGoing = IsCorrect (index, guess, currentCard, nextCard, valueOrder, suitOrder, ref shownValues, ref shownSuits); //update read
        if(keepGoing==true) wins++;
    }
    // when the guess is wrong, still flip the next card
    index ++;
    currentCard = rowOfTen[index];
    FlipCardInRow(index, currentCard, ref shownValues, ref shownSuits);
...
static bool IsCorrect(int index, string guess, string currentCard, string nextCard, string[] valueOrder, string[] suitOrder, ref string[] shownValues, ref string[] shownSuits){
    //Compare the cards:
    ...
    if (guess.ToLower() == result){
        ...
        System.Console.WriteLine($"Card #{index+2} is {nextCard}.");
        return true;
    }
    else {
        Console.Clear();
        System.Console.WriteLine("Your guess is wrong! You lose!");
        System.Console.WriteLine($"Card #{index+2} is {nextCard}. The correct answer should be '{result}'.");
        return false;
    }
}
